...
def fetch_data():
    # Retrieve current weather data for Mumbai from Visual Crossing API
    ...
    url = f"https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/Mumbai?unitGroup=metric&key={api_key}&fields={fields}"
    response = requests.get(url)
    data = response.json()

    # Store data in MongoDB Atlas
#    collection_name = f"Mumbai_{datetime.now().strftime('%B_%Y')}"
    collection_name = f"Mumbai_{datetime.now().strftime('%m_%Y')}"
    collection = db[collection_name]
    collection.insert_one(data)

    print(f"Stored data for {datetime.now().strftime('%Y-%m-%d')}")

# Fetch current weather data for Mumbai and store it in MongoDB Atlas
fetch_data()
