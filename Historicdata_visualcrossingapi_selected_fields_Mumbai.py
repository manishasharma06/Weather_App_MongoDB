...
api_key = "5XF88HMPTMB78DXWCYCYPL68Y"
...
def fetch_data(date):
    # Retrieve historical weather data for Mumbai from Visual Crossing API
    ...
    url = f"https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/Mumbai/{date_str}?key={api_key}&unitGroup=metric&include=obs&fields={fields}"
    response = requests.get(url)
    data = response.json()

    # Store data in MongoDB Atlas
    collection_name = f"Mumbai_{date.strftime('%m_%Y')}"
    collection = db[collection_name]
    collection.insert_one(data)

    print(f"Stored data for {date_str}")

# Fetch historical weather data for Mumbai for the year 2022 and store it in MongoDB Atlas
